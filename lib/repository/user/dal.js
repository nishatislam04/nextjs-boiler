...
import { CACHE_TTL, CACHE_TTL_SHORT, PER_PAGE } from "@/lib/settings";
...
export async function fetchUserPosts(
	...
	const cacheKey = helpers.buildCacheKey("privatePost:listings", {
		currentPage,
		id,
		orderBy,
		query,
...
export async function fetchUserProfile(id) {
	const cacheKey = helpers.buildCacheKey("user:profile", {
		id,
	});
	let user = await redis.get(cacheKey);
